...
class ThreadTestCase(TestCase):
	...
	def test_add_messages_to_thread(self):
		self.thread.users.add(self.user1, self.user2)
		message1 = Message.objects.create(user=self.user1, content="epale")
		message2 = Message.objects.create(user=self.user2, content="que hay")
	...
	def test_add_messages_from_user_not_in_thread(self):
		...
		message1 = Message.objects.create(user=self.user1, content="epale")
		message2 = Message.objects.create(user=self.user2, content="que hay")
		message3 = Message.objects.create(user=self.user3, content="hola soy un espia XD")
